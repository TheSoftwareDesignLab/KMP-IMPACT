...
| [Confetti](confetti.md) | AGP 8.13 + Gradle 9.3.1 + Kotlin 2.2.21 | Largest tree (299 Kotlin files). Has heavy upstream workflows that must be disabled before adoption. |
| [DroidconKotlin](droidcon-kotlin.md) | Gradle 8.14.3 + Kotlin 2.2.21 | Best F1 of the evaluation (0.828). Has 2 upstream workflows to disable. |
| [KMP-App-Template](kmp-app-template.md) | AGP 8.5 + Gradle 9.3.1 + Kotlin 1.9.24 | Compose Multiplatform (`org.jetbrains.compose.*`) — currently lowest F1 due to mapping gap. |
| [kmp-production-sample](kmp-production-sample.md) | AGP 8.1.4 + Gradle 8.5 + Kotlin 1.9.21 | Multi-target (Android, iOS, jvm, js). Recall 1.0. |
...
4. If your Android module is not detected automatically (see [Architecture](../architecture/overview.md)), set the env var `KMP_IMPACT_ANDROID_MODULE=":your-module"` in the workflow.
